#!/bin/bash
# T98 follow-on (2026-09-20): the 11 near-threshold committed designs that T78 re-solved on a
...
log(){ echo "[$(date '+%F_%T')] $*" >> $L/grid_delta3_server.log; }
log "stage: 11 subsampled near-tau designs, grid 256, all 100 wavelengths, 2 workers"
QUEUE=scripts/grid_delta_queue3.txt
  ...
  while read s seed i; do [ -z "$s" ] && continue
    if [ $((n % 2)) -eq $w ]; then log "w$w start $s s$seed i$i"
      $PY scripts/grid_delta_resolve.py $s $seed $i --n-wl 0 --grids 256 --device cuda \
         --out results_pub/grid_delta_full >> $L/grid_delta3_w$w.log 2>&1
      log "w$w done  $s s$seed i$i (exit $?)"; fi
    n=$((n+1)); done < $QUEUE; }
run_worker 0 & sleep 120; run_worker 1 & wait
log "ALL DONE"; echo DONE > $L/grid_delta3_DONE.txt
