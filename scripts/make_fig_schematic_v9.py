...
texts = ["1  TMM-pretrained checkpoint [2]",
         ...
         "6  RCWA oracle at the committed geometry\n(order 5; order 7 for seed 42)",
...
tb = ["T1  RCWA MAE > 3 × surrogate MAE",
      ...
      "T3  NN distance < 0.5 × median pair distance\n(vs N = 20 null)",
      "T4  K = 100 perturbations, σ = 1 % box width;\nflag if mean MAE > 5 %"]
for i, t in enumerate(tb):
    box(axb, 0.02, 0.80 - i * 0.2, 0.96, 0.16, t, FILL[3], fs=6.5)
axb.text(0.5, 0.03, "τ = 5 %, k = 2 (Δ-flag threshold = k × forward MAE)", ha="center", va="bottom", fontsize=6.5)
