...
def load_and_preprocess(npz_path: str, structure: str, metal: str = "Cr",
    ...
    # Reliability mask (M1): per (sample, wavelength) bool flagging physically
    # reliable RCWA points; grazing/Wood's-anomaly points are False and must be
    # excluded from training/eval. Backward-compatible: npz without the field
    # (legacy datasets) -> treat all points as reliable.
    if structure == "C":
        rel_te = (d["reliable_TE"].astype(bool) if "reliable_TE" in d.files
                  else np.ones((N, Nlam), dtype=bool))
        rel_tm = (d["reliable_TM"].astype(bool) if "reliable_TM" in d.files
                  else np.ones((N, Nlam), dtype=bool))
    else:
        rel = (d["reliable"].astype(bool) if "reliable" in d.files
               else np.ones((N, Nlam), dtype=bool))
    ...
    if structure == "A":
        ...
        # Legacy isotropic Structure-C loader, used only by the isotropic/scalar C
        # experiments. The paper's dual-polarization Structure-C result is generated by
        # pbtl_C_v2_redesign.py via src.simulation.tmm_struct_c_aniso (anisotropic TE/TM
        # EMA), not this path.
    ...
    def _build_split(sub_idx):
        ...
        if structure == "C":
            ...
            for ch, arr in [("A_tmm_te", A_tmm_te), ("R_tmm_te", R_tmm_te), ("T_tmm_te", T_tmm_te),
                ...
                result[ch] = torch.tensor(_subset(arr.reshape(-1), sub_idx))
            # M1: per-polarization reliability mask (bool), aligned with the channels
            result["mask_TE"] = torch.tensor(_subset(rel_te.reshape(-1), sub_idx))
            result["mask_TM"] = torch.tensor(_subset(rel_tm.reshape(-1), sub_idx))
        else:
            for ch, arr in [("A", A), ("R", R), ("T", T)]:
                result[ch] = torch.tensor(_subset(arr.reshape(-1), sub_idx))
            for ch, arr in [("A_tmm", A_tmm), ("R_tmm", R_tmm), ("T_tmm", T_tmm)]:
                result[ch] = torch.tensor(_subset(arr.reshape(-1), sub_idx))
            # M1: reliability mask (bool), aligned with the flattened rows
            result["mask"] = torch.tensor(_subset(rel.reshape(-1), sub_idx))
        return result
